...
<head>
<meta charset="utf-8">
<title>Quiniela_Mundial_2018/Adm_calendario</title>
...
<section>
<h2 style="margin-left:40px">Plantilla para Ver Códigos de inicio Usados y Borrar Quinielas.</h2>
<h4 style="margin-left:1px">Si elimina una Quiniela el Usuario podrá llenarla nuevamente entrando como NUEVO USUARIO con este mismo Codigo de Inicio.</h4>
<h3 style="margin-left:1px">IMPORTANTE: Tenga en cuenta que una vez que oprima el botón "BORRAR" se perderán los datos de usuario y quiniela para ese Codigo de Inicio.</h3>
<table style="width:700px;">
  <tr>
    <td>ID</td>
    <td style="width:200px;">Código de Inicio</td>
...
try{
	...
	$consulta="SELECT ID FROM `datos_usuarios`";
	...
	$consulta="SELECT * FROM `datos_usuarios` ORDER BY ID LIMIT $empesar_desde,$registros_filtrados";
	$resultados=mysqli_query($conexion,$consulta);
	while(($fila=mysqli_fetch_array($resultados))==true){
		$id=$fila['ID'];
		$cod_ini=$fila['CODIGO_INICIO'];
		$usuario=$fila['USUARIO'];
		$correo=$fila['CORREO'];
	echo "<form action='QMF2016_Adm_Tabla_Usuarios_Delete.php' method='post'><tr>";
	echo "<td>$id<input type='hidden' name='id' value='$id'><input type='hidden' name='pagina' value='$pagina'><input type='hidden' name='cod_ini' value='$cod_ini'><input type='hidden' name='usuario' value='$usuario'><input type='hidden' name='correo' value='$correo'></td>";
	echo "<td><input type='text' disabled style='width:170px; background-color:#FFF; color:#000;' name='falso01' value='$cod_ini'></td>";
	echo "<td><input type='text' disabled style='width:170px; background-color:#FFF; color:#000;' name='falso02' value='$usuario'></td>";
	echo "<td><input type='text' disabled style='width:170px; background-color:#FFF; color:#000;' name='falso03' value='$correo'></td>";
	echo "<td><input type='submit' name='borrar' value='Borrar'></td></form>";
	echo "</tr>";
	}
	...
	echo "<tr><td colspan='5'>Página Actual: $pagina - Páginas disponibles: ";
	
	if($pagina<=10 or $pagina>=$total_paginas-10){
		for($i=1; $i<=10; $i++){
			echo "<a href='?pagina=" . $i . "'>" . $i . "</a>&nbsp;&nbsp;";
		}
		echo "&nbsp;---&nbsp;";
		for($i=20; $i<=$total_paginas-10; $i=$i+15){
			echo "<a href='?pagina=" . $i . "'>" . $i . "</a>&nbsp;&nbsp;";
		}
		echo "&nbsp;---&nbsp;";
		for($i=$total_paginas-10; $i<=$total_paginas; $i++){
			echo "<a href='?pagina=" . $i . "'>" . $i . "</a>&nbsp;&nbsp;";
		}
	}else{}
	if($pagina<$total_paginas-10 and $pagina>10){
		for($i=1; $i<=$pagina-6; $i=$i+15){
			echo "<a href='?pagina=" . $i . "'>" . $i . "</a>&nbsp;&nbsp;";
		}
		echo "&nbsp;---&nbsp;";
		...
		echo "&nbsp;---&nbsp;";
		for($i=$pagina+6; $i<=$total_paginas; $i=$i+15){
			echo "<a href='?pagina=" . $i . "'>" . $i . "</a>&nbsp;&nbsp;";
		}
	}else{}

	echo "<br>$registros_filtrados Codigos mostrados de $numero_filas en total.</td></tr>";
?>
